...
from extraction.utils import (
    ...
    align_extracted_users_by_registry, parse_header_group, build_group_col_mappings,
...
def split_multi_value_field(val: str, delimiter: str = None) -> list[str]:
    """
    Splits a multi-value string by the specified delimiter (pipe, newline, comma, semicolon).
    If no delimiter is specified, auto-detects between pipe and newline.
    Cleans up numbered/bullet prefixes and trailing commas/semicolons.
    """
    if not val:
        return []
    ...
    if delimiter:
        parts = [p.strip() for p in val_str.split(delimiter)]
    elif '|' in val_str:
...
def detect_multi_user_delimiter(user: dict) -> str:
    """
    Detects if there is a consistent multi-user delimiter (e.g. '|', '\n', ',', ';')
    that appears in multiple key fields of the same user row.
    """
    delimiters = ['|', '\n', ',', ';']
    fields_to_check = ['firstName', 'lastName', 'employeeId', 'email', 'phone']
    
    delim_counts = {d: 0 for d in delimiters}
    for d in delimiters:
    ...
    # If a delimiter appears in at least 2 distinct key fields, it is a multi-user delimiter!
    for d in delimiters:
        if delim_counts[d] >= 2:
            return d
            
    # Fallback to '|' or '\n' if present in even one of the main fields
    for d in ['|', '\n']:
        for f in ['firstName', 'employeeId']:
            if d in str(user.get(f, '')):
                return d
...
def local_extract_users(file_bytes, filename, pass_prefix="Med", user_intent=""):
    ...
    for sheet_name, raw_df in all_sheets.items():
        ...
        group_mappings = build_group_col_mappings(headers, user_intent=user_intent)
        
        # Check for running/floating roles columns in headers (must not be a tick-marked column)
        roles_col_names = []
        for h in headers:
            is_mapped = any(h in mapping for mapping in group_mappings.values())
            if is_mapped:
                continue
        ...
        for src_col in headers:
            ...
            is_mapped = any(src_col in mapping for mapping in group_mappings.values())
            if is_role_header and not is_mapped and src_col not in roles_col_names:
        ...
        for _, row in data_df.iterrows():
            for g_id, col_mapping_g in group_mappings.items():
                ...
                for r_col in roles_col_names:
                    _, r_g = parse_header_group(r_col)
                    if r_g != g_id:
                        continue
                ...
                for src_col, target_field in col_mapping_g.items():
                ...
                if role_cols:
                    ...
                    for rc_col, rc_name in role_cols.items():
                        _, rc_g = parse_header_group(rc_col)
                        if rc_g != g_id:
                            continue
                ...
                # --- SPLIT MULTI-USER ROWS ---
                delim = detect_multi_user_delimiter(user)
                ...
                if delim:
                    ...
                    for f in identity_fields:
                        val = str(user.get(f, ''))
                        parts = split_multi_value_field(val, delimiter=delim)
